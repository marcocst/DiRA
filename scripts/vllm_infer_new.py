...
def vllm_infer(
    ...
    # cutoff_len: int = 2048, 原始的
    cutoff_len: int = 200,
    max_samples: Optional[int] = None,
    vllm_config: str = "{}",
    save_name: str = "generated_predictions_2025_8_6_111.jsonl",
    temperature: float = 0,     # 奥对，这里有个temperature，这个要设置的。默认值是0.95, 确实设置为0就不会变了
    top_p: float = 0.7,
    top_k: int = 50,
    # max_new_tokens: int = 1024, 原始的
    ...
    llm = LLM(
    model="output/llama3_lora_sft",
    max_num_seqs=16,  # 大幅降低默认值（原为256）
    gpu_memory_utilization=0.8,  # 适当降低显存占用比例
    )
